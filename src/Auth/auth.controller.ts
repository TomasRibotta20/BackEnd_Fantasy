...
function sanitizeRegisterInput(req: Request, res: Response, next: NextFunction) {
  ...
  //Ahora la idea es que si el usuario ingresa para un register, entonces el role no lo propociona, pero si es un admin, entonces puede especificar el role
  // Si no se especifica, por defecto será 'user'
  if (req.authUser.user !== null) {
    if (req.body.role && req.authUser.user!.role === 'admin') {
      const role = req.body.role;
      if (role !== 'user' && role !== 'admin') {
        return res.status(400).json({ message: 'Invalid role' });
      }
      req.body.sanitizedInput.role = role;
    }
  }
  next();
}
...
async function login(req: Request, res: Response) {
  ...
  try {
    ...
    return res.cookie('access_token', token, {
      httpOnly: true,
      sameSite: 'strict',
      maxAge: 1000 * 60 * 60 }).status(200).json({ message: 'Login successful', data: userWithoutPassword }
    ); //Falta activar el secure en producción
  } catch (error) {
